  ...
  <div class="assist-records">
    <el-card>
      ...
      <el-table :data="tableData" v-loading="loading" border>
        ...
        <el-table-column prop="optionCode" label="选项" width="100">
          <template slot-scope="scope">
            <el-tag type="info" size="small">{{ getOptionText(scope.row.optionCode) }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="assistValue" label="助战值" width="80" align="right" />
        <el-table-column prop="costGold" label="消耗金币" width="100" align="right">
          <template slot-scope="scope">
            <span :class="scope.row.isSystemUser ? '' : 'gold-text'">
              🪙 {{ scope.row.costGold }}
            </span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="createdAt" label="助战时间" width="180" />
      </el-table>
...
<script>
import { findAssistRecordList } from '@/api/battleAdminService'
  ...
  methods: {
    async loadData() {
      try {
        ...
        const res = await findAssistRecordList(req)
        if (res.code === 'Success') {
