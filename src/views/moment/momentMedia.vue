<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="Thumbnail" width="150px" align="center" prop="thumbnail" />
      <el-table-column label="Title" width="150px" align="center" prop="title" />
      <el-table-column label="Artist" width="150px" align="center" prop="artist" />
      <el-table-column label="Ablum" width="150px" align="center" prop="ablum" />
      <el-table-column label="Duration" width="150px" align="center" prop="duration" />
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      >
        ...
        <el-form-item label="Ablum" prop="ablum">
          <el-input v-model="temp.ablum" />
        </el-form-item>
...
<script>
import { createMomentMedia, updateMomentMedia, deleteMomentMedia, findMomentMediaById, findMomentMediaList } from '@/api/momentMedia'
import waves from '@/directive/waves' // waves directive
import Pagination from '@/components/Pagination' // secondary package based on el-pagination
import tableList from '@/mixins/tableList'

export default {
  name: 'MomentMediaTable',
  components: { Pagination },
  directives: { waves },
  mixins: [tableList],
  data() {
    return {
      listApi: findMomentMediaList,
      tableKey: 0,
      temp: {
        id: undefined,
        createdAt: '',
  ...
  methods: {
    ...
    async createData() {
      this.$refs['dataForm'].validate(async(valid) => {
        if (valid) {
          const res = await createMomentMedia(this.temp)
          if (res.code === 'Success') {
            this.handleFilter()
    ...
    async handleUpdate(row) {
      const res = await findMomentMediaById({ id: row.id })
      console.log(res)
      if (res.code === 'Success') {
        this.temp = res.data
    ...
    async updateData() {
      this.$refs['dataForm'].validate(async(valid) => {
        if (valid) {
          const res = await updateMomentMedia(this.temp)
          if (res.code === 'Success') {
            this.dialogFormVisible = false
            this.$notify({
              title: 'Success',
    ...
    async handleDelete(row) {
      await deleteMomentMedia({ id: row.id })
      this.getTableData()
    }
  }
}
