...
<script>
import { createMomentLike, updateMomentLike, deleteMomentLike, findMomentLikeById, findMomentLikeList } from '@/api/momentLike'
import waves from '@/directive/waves' // waves directive
import Pagination from '@/components/Pagination' // secondary package based on el-pagination
import tableList from '@/mixins/tableList'

export default {
  name: 'MomentLikeTable',
  components: { Pagination },
  directives: { waves },
  mixins: [tableList],
  data() {
    return {
      listApi: findMomentLikeList,
      tableKey: 0,
      temp: {
        id: undefined,
        createdAt: '',
  ...
  methods: {
    ...
    async createData() {
      this.$refs['dataForm'].validate(async(valid) => {
        if (valid) {
          const res = await createMomentLike(this.temp)
          if (res.code === 'Success') {
            this.handleFilter()
    ...
    async handleUpdate(row) {
      const res = await findMomentLikeById({ id: row.id })
      console.log(res)
      if (res.code === 'Success') {
        this.temp = res.data
    ...
    async updateData() {
      this.$refs['dataForm'].validate(async(valid) => {
        if (valid) {
          const res = await updateMomentLike(this.temp)
          if (res.code === 'Success') {
            this.dialogFormVisible = false
            this.$notify({
              title: 'Success',
    ...
    async handleDelete(row) {
      await deleteMomentLike({ id: row.id })
      this.getTableData()
    }
  }
}
